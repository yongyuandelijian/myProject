...
waiting()
{
      while true
            do
                ddlsql="select count(*) as result from odps_jobstatus t where substr(t.jobname,1,8) >= '${2}' and projectname = '${1}' and job_status = 'OK' and filesymbolgs = 'OK2' AND filename NOT LIKE '${bizdate}%'"
