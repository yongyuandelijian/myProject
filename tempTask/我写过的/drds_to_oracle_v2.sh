...
# 参数：表名称 (hx_dm_zdy_dm_gy_swjg)
# 用法：sh makejson_drds_oracle.sh hx_dm_zdy_dm_gy_swjg '20200917 0920'
# 思路：先利用sfq抽取二十分钟之前的数据增量到临时表，然后再使用oracle的更新思路进入到最终表

# 定义普通参数
src_tabname=$1
curdir=`pwd`
# curtime=$(date "+%Y%m%d%H%M%S") 为了测试我们先使用一个指定的值
curtime=$2
pretime=$(date -d  "${curtime} 20 minute ago" "+%Y%m%d%H%M")
...
drdsinfo=$(mysql -h127.0.0.1 -P3306 -u1111 -p1111 -D rcount -N -e "${sql_getdrds}")
...
datax_where="sfq>='${pretime}' and sfq<='${curtime}'"	# 获取where 条件 drds不需要配置主键切分
...
#echo -e "$json">${mydir}/${sstable}_whts.json
#sed -i "s/00010101,yyyymmdd/'00010101','yyyymmdd'/g" ${mydir}/${sstable}_whts.json
#sed -i "s/99991231235959,yyyymmddhh24miss/'99991231235959','yyyymmddhh24miss'/g" ${mydir}/${sstable}_whts.json
#sed -i 's/"date"/"\\"date\\""/g;s/"timestamp"/"\\"timestamp\\""/g' ${mydir}/${sstable}_whts.json
#sed -i 's/ date / "\\"date\\"" /g;s/ timestamp / "\\"timestamp\\"" /g' ${mydir}/${sstable}_whts.json
