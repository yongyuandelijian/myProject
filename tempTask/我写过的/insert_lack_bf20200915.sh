...
# author：aaa
# date: 20200810
# 修改：20200905 有时候需要跑的表多，所以增加并发处理，增加延迟检测
# 参数：业务日期（20200810）  单位代码（SXST）  表单从[单位代码.list]中读取 (HX_FP_FP_DCXCQDB_MX)
# 用法：nohup sh insert_lack_bf.sh 20200831 SXST >insert_lack.log 2>&1 &
# 修改：20200921 增加多主键处理
#####################################################################################################

# 如果参数个数不匹配则提示校验，防止带入造成较大问题
if [ $# -ne 2 ];then echo "要求的参数个数是bizdate和dwmc共2个，当前提供的参数是$#个，分别是$*请检查核对！！！"; exit -1; fi;
