...
class get_ybwb(object):
    ...
    def hqnr(self):
        ...
        jb1_2 = "第2章\t数据集成情况说明"
        jb2_21 = "2.1\t链路介绍"
        nr2101 = "目前总局云平台数据采集链路及分层架构，基于目前总局、省级单位相关税收业务数据、外部交换数据、出口退税、电子底账和互联网采集等数据的分布特性，云平台目前共有12条数据集成链路，根据不同的数据来源，我们采用相对应的技术工具，将不同位置不同业务的数据源数据首先采集至云平台各项目独立存储区即镜像层，再按需同步至税务总局基础层，个别临时提供的数据直接集成到税务总局基础层，最终实现云平台基础数据层数据的采集。"
        ...
        nr01 = "主要包含了核心征管系统的申报、征收、登记、发票、纳评、票证、认定、税服、优惠、证明、代码参数等业务域数据，旧个税系统的登记、申请、认定、征收、代码等业务域数据，新出口退税系统（已切换6家单位）的备案、审核、风险、评估、申报、证明、代码参数等业务域数据；省局社保费子系统（已试点5家单位）的社保费征收、管理、申报等数据； 好差评系统的评价意见等数据。此链路中集成的数据均是从源端将生产库数据同步到各地核心征管分发库中，云平台再通过在每个单位分发库部署的OGG工具以增量文件形式将数据文件获取到云平台服务器，每日再通过配置云平台调度任务将日志文件数据加载到ODPS对应表中，每日增量数据先进入云平台各单位临时层，再merge到云平台基础层供下游使用。"
        ...
        jb4_210204 = "2.1.2.4\t链路5：出口退税数据集成"
        ...
        jb4_210205 = "2.1.2.5\t链路6：总局其它应用系统数据集成"
        ...
        jb4_210206 = "2.1.2.6\t链路7：互联网爬取数据集成"
        nr01 = "以总局为节点采集政府公开公示、企业官网以及财经新闻门户、东方财富网股东信息等互联网信息，集成后进行结构化再集成到云平台镜像层，按需同步到基础层供下游使用，此类数据为按需集成。"
        jb4_210206 = {jb4_210206: [nr01]}
        jb4_210207 = "2.1.2.7\t链路8：外部临时导出数据集成"
        nr01 = "针对一些临时需求从外部其它系统临时导出的相关数据进行云平台导入集成，目前已经集成的有公安自然人身份信息数据、宁波市各部门个税测算数据、风险司海关注册企业信息,最高法裁决文书、收规司社保数据、大企业及千户集团数据、所得税司商品清单,稽查局违法案件数据、纳服司涉税举报数据、税务审计软件数据、收规司企业出口情况数据、湖南工商比对核实数据、网信办复工复产数据、所得税司高新企业名单等，此部分数据经常以文件形式提供，单次按需集成，所有数据均直接进入云平台基础层，供下游直接加工使用。"
        jb4_210207 = {jb4_210207: [nr01]}
        jb4_210208 = "2.1.2.8\t链路9：老核心征管系统历史数据集成 "
        nr01 = "针对总局电税中心留存的核心管理历史数据（CTAIS1.1、2.0）进行采集集成，此部分数据也是独立需求，在云平台做为独立项目进行数据分析使用，因此数据存放于自己独立镜像层下进行加工分析使用。"
        jb4_210208 = {jb4_210208: [nr01]}
        jb4_210209 = "2.1.2.9\t链路10：电子商务数据集成"
        ...
        jb4_210210 = "2.1.2.10\t链路11：所得税汇算清缴系统数据集成"
        ...
        jb4_210211 = "2.1.2.11\t链路12：自然人电子税务局系统数据集成"
