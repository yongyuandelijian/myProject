...
import matplotlib.pyplot as plt   # 如果需要例子，可以去https://matplotlib.org/gallery/index.html
...
class scword():
    ...
    def __init__(self,ny):
        self.ny=ny
        self.wdmc=r'E:\work\GT3-SJZY-YPT-云平台数据管理子项目-数据集成运维报告-{ny}.docx'.format(ny=ny)   # 要求必须传入文档名称
                    ...
                    for ej in xl.keys():
                        ...
                        for nr in xl.get(ej):
                            ...
                            if ej=="3.2\t分发库运行情况\t\t" and num==1:
                                ...
                                doc.add_paragraph("\n")
                                # 添加折线图
                                plt.plot(ycs,linewidth=1)  # 传入列表绘制图形,并且设置线条宽度
                                bt="图3.2.1 {ny}分日分发库异常进程情况统计图".format(ny=self.ny)
                                plt.title(bt,fontsize=18)  # 设置标题和标题字体大小
                                plt.xlabel("日期",fontsize=16)
                                plt.ylabel("异常数(次)",fontsize=16)
                                plt.tick_params(labelsize=12)  # 添加刻度
                                plt.savefig(r"..\img\img_nr3201_ffkyc_frq.png")
                                # plt.show()  # 现在不需要展示将保存好的图片插入到word中
                            ...
                            if ej =="4.5\t数据集成一致性监控处理情况\t\t":
                                if num ==1:
                                    ...
                                    for tablename in list_tablename:
                                        ...
                                        bt = workbook.add_format({'bold': True,'bg_color':'1C98C6','align':'centre'})
                                        ...
                                        for row_num in range(len(data)):
                                            for col_num in range(len(data[0])):
                                                worksheet.write(row_num+2,col_num,data[row_num][col_num])
                                    workbook.close()
                                    # 将生成好的Excel附加进来
                                elif num ==3:
