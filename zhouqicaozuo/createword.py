...
class scword(Common):
    ...
    def scword(self):
        ...
        doc=self.doc
        # 设置pyecharts读取的路径
        js_path = os.path.join(self.root_dir, r'js/')
        ...
        run=ymdl.add_run()
        img_path=os.path.join(self.root_dir,'img/img_ym01.png')
        run.add_picture(img_path,width=Cm(2.49),height=Cm(0.9))
        ymnr='\t'+' '*45+'2021年云平台运行维护及优化完善项目-数据层运维统计分析月报'
        run.add_text(ymnr)
        font=run.font
        ...
        dljg=ztdx_jb1+2
        paragraph_format.space_after=Pt(dljg*6)
        paragraph_format.space_before=Pt(dljg*3)
        ...
        for jb1 in all_text.keys():
            ...
            for jb2 in all_text.get(jb1):
                for jb2key in jb2.keys():
                    self.addTitle(doc,ztdx_jb2,jb2key,1.73,16,2,u'黑体')
                    if jb2key[:jb2key.index("\t")] == "2.1":
                        # 处理直属内容
                        img_path = os.path.join(self.root_dir, 'img/img_dl21.png')
                        doc.add_picture(img_path, width=Inches(6.25))  # 这个数字必须指定
                        for nr in nr21:
                            ...
                            for jb3key in jb3.keys():
                                ...
                                for jb4 in jb3.get(jb3key):
                                    for jb4key in jb4.keys():
                                        self.addTitle(doc,ztdx_jb4,jb4key,1.57,16,4)
                        ...
                        tr_text = ["分类", "来源系统", "表数量", "数据量（万）", "存储（GB）", "集成频率", "集成方式","所属链路","抽取状态",'是否新增']
                        tr_width=[1,1.3,1,1.5,1.2,1,1,1,1,1]
                        ...
                        for nr in jb2.get(jb2key):
                            if temp==1:
                                img_path = os.path.join(self.root_dir, 'img/img_dl23.bmp')
                            ...
                            for jb3key in jb3.keys():
                                ...
                                if jb3key[:jb3key.index("\t")]=="3.1.1":
                                    ...
                                    for mx in data:
                                        day.append(mx[0])
                                        ycs.append(mx[1])
                                    bt="图3.2.1 {ny}分日任务异常进程情况统计图".format(ny=self.ny)
                                    ...
                                    img_path = os.path.join(self.root_dir, 'img/img_b321_rwyc_frq.png')
                                    make_snapshot(snapshot, zxt321.render(), img_path)
                                    self.doc.add_picture(img_path, width=Inches(6.25))
                                    ...
                                    nr="{ny}整月集成问题共出现{zcs}次，共涉及{dws}家单位，分别是：{temp}".format(ny=self.ny,zcs=zcs,dws=dws,temp=temp)
                                    self.addParagraph(nr,ztdx_nr,doc)
                                    nr="区域地图显示如下："
                                    self.addParagraph(nr,ztdx_nr,doc)
                                    ...
                                    img_path = os.path.join(self.root_dir, 'img/img_nr321_rwycfdw.png')
                                    make_snapshot(snapshot, m1.render(), img_path)
                                    self.doc.add_picture(img_path, width=Inches(6.25))
                                    ...
                                    if len(yccs) > 0:
                                        ...
                                        img_path = os.path.join(self.root_dir, 'img/img_t322_rwyc_fyy.png')
                                    ...
                                    for gdnr in jb3.get(jb3key):
                                        if temp_num==2:
                                            temp = "以下选取5张金三核心表对{yf}月24号至{yf}月28号5天数据抽取情况进行数据量分析比对，详情见附件：《{ny}数据抽样比对结果.xlsx》".format(yf=yf,ny=self.ny)
                                            self.addParagraph(temp,ztdx_nr,doc)
                                            # 生成excel
                                            temp_path=r"appendix/{ny}数据抽样比对结果.xlsx".format(ny=self.ny)
                                            excel_path = os.path.join(self.root_dir, temp_path)
                                            ...
                                            workbook.close()
                                        if temp_num==3:
                                            temp = "另本月运维和数据差异的处理记录，详情见附件：《{ny}数据差异运维记录.xlsx》。".format(ny=self.ny)
                                            self.addParagraph(temp,ztdx_nr,doc)
                                            # 生成 4.5 excel 2 数据差异运维记录
                                            temp_path = r"appendix/{ny}数据差异运维记录.xlsx".format(ny=self.ny)
                                            excel_path = os.path.join(self.root_dir, temp_path)
                                            workbook = xlsxwriter.Workbook(excel_path)
